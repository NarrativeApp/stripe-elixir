defmodule Stripe.CustomerTest do
  ...
  test "list all customers" do
    assert {:ok, %{"object" => "list", "url" => "/v1/customers"}} = Customer.list()
  end
  ...
  test "list cards", %{customer: customer} do
    ...
    assert {:ok, %{"data" => data, "object" => "list", "url" => ^customer_sources_url}} =
  ...
  test "add/update/delete a card to a customer", %{customer: customer} do
    ...
    assert {:ok, %{"deleted" => true}} = Customer.delete_card(customer["id"], source_id)
  end
